...
class PhoMTDataset(Dataset):
    def __init__(self, path: str, vocab: Vocab, src_language: str, tgt_language: str):
        super().__init__()
        self.vocab = vocab
...
def collate_fn(items, vocab, src_language, tgt_language):
    src_sents = [item[src_language] for item in items]
    tgt_sents = [item[tgt_language] for item in items]
    
    src_padded = pad_sequence(src_sents, batch_first=True, padding_value=vocab.pad_idx)
    tgt_padded = pad_sequence(tgt_sents, batch_first=True, padding_value=vocab.pad_idx)
